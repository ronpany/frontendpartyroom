 ...
 * @param {*} infCliente informacion adicional del cliente
 */
 function mostrarEliminar(idCliente,infCliente){
    $("#nuevo").hide();
    $("#editar").hide();
    $("#listado").hide();
    $("#nuevoRegistro").hide();
    $("#titleIdDelete").html("Desea eliminar el cliente con el id: " + idCliente + " ?...");    
    $("#idDelete").val(idCliente);
    $("#clientDelete").html(infCliente);
    $("#eliminar").show(1000);
}
/*
...
function borrarRegistro(llaveRegistro) {
    ...
    $.ajax({
        ...
        error: function (xhr, status) {
            $("#mensajes").html("El Cliente que intenta eliminar tiene información relacionada, por favor verifique...");
            $("#mensajes").show(1000);
        }
    });
}