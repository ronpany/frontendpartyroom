 ...
 * @param {*} idBike codigo de la Botecleta
 * @param {*} infCostume informacion adicional de la Botecleta
 */
 function mostrarEliminar(idBike,infBike){
    $("#nuevo").hide();
    $("#editar").hide();
    $("#listado").hide();
    $("#nuevoRegistro").hide();
    $("#titleIdDelete").html("Desea eliminar La Botecleta con el id: " + idBike + " ?...");    
    $("#idDelete").val(idBike);
    $("#bikeDelete").html(infBike);
    $("#eliminar").show(1000);
}
/*
...
function borrarRegistro(llaveRegistro) {
    ...
    $.ajax({
        ...
        error: function (xhr, status) {
            $("#mensajes").html("La Botebleta que intenta eliminar tiene información relacionada, por favor verifique...");
            $("#mensajes").show(1000);
        }
    });
}